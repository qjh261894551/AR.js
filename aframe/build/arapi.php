...
// $fetch=pdo_fetch("SELECT levenshtein('iossdasdasd',source) as levenshtein FROM".tablename('mcar_goods_imgInfo')."WHERE  id=3"." ORDER BY id ASC limit 1");
// exit($fetch['levenshtein']);
$key='abc';//约定key
if(isset($_GET['match'])){
	if($_GET['match']!=$key){
...
if(isset($_GET['data'])){
	...
	$histogram32Str = $data->histogram32Str;
	...
	switch ($data->type) {
		case 'stuff':
			$result=saveStuffImgInfo($histogram32Str,$hashData,$mydata,$location,$data->title,$url,$grayData,$source);
...
function saveStuffImgInfo($histogram32Str,$hashData,$data,$location,$title,$url,$grayData,$source)//$data数组由16个数字构成，$location为GPS经纬度，
	{
